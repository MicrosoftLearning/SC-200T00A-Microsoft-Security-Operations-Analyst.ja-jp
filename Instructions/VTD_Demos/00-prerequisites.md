...
このコースのラボには、Microsoft Defender for Endpoint P2 ライセンスと Azure サブスクリプションを持つ Microsoft 365 E5 ライセンス テナントが必要です。

* SC-200 Microsoft Security Operations Analyst コースと同様に、これらのデモは承認されたラボ ホスティング環境で実行されるように設計されています。
...
>**注:** 前述のとおり、Microsoft 365 E5 テナントに接続されている Azure サブスクリプションを必要とするデモがあります。 自分の Azure サブスクリプションと Microsoft 365 E5 テナントを使用できます。 [Microsoft 365 開発者プログラム](https://developer.microsoft.com/microsoft-365/dev-program/) Web サイトにアクセスしてメンバーシップを要求し、無料の Microsoft 365 E5 開発者向けサブスクリプションを設定してください。

## Defender for Endpoint のデプロイ

### Microsoft 365 資格情報の取得

ホストされたラボを起動すると、Microsoft 仮想ラボ環境で無料試用版テナントが利用できるようになります。 このテナントには自動的に一意のユーザー名とパスワードが割り当てられています。 Microsoft Virtual Lab 環境で Azure と Microsoft 365 にサインインするには、このユーザー名とパスワードを取得する必要があります。

このコースは、学習パートナーが複数の認証済みラボ ホスティング プロバイダーのいずれかを使用して実施することもあり、お使いになっているテナントに関連のあるテナント ID を取得する実際の手順はラボ ホスティング プロバイダーに応じて異なります。 このため、コース用のこの情報の取得方法については、講師が必要な指示を行います。 後ほど使用できるよう以下の情報に留意してください。
...
1. Microsoft Edge ブラウザーで、(https://security.microsoft.com)) の Microsoft Defender ポータルに移動します。
...
1. 新しいワークスペースの初期化が正常に完了すると、**[ホーム]** ポータル ページに、**[SIEM と XDR を 1 か所で取得する]** バナーが表示されます。 また、**[設定]** では、アカウント、メール通知、**プレビュー機能**、アラート サービス設定、アクセス許可、ロール、およびストリーミング API の Microsoft Defender XDR 全般設定がオンになります。

1. 新しいスペースが正常に完了すると、アカウント、電子メール通知、アラート サービス設定、アクセス許可、ロールおよび Streaming API の Microsoft 365 Defender [全般] 設定が表示されます。 **[プレビュー機能]** もオンになります。
...
1. から始めます。 [デバイス] タブの OS 条件で、**[Windows 10]** を選択し、**[次へ]** を選択します。
...
このタスクでは、Microsoft Sentinel ワークスペースを作成します。
 ...
 >**注:** 次のデモでは、Azure サブスクリプションがアクティブである必要があります。
...
1. ブラウザを開き、新しいMicrosoft Edgeブラウザーを検索、ダウンロード、およびインストールします。 新しい Microsoft Edge ブラウザーを起動します。
...
1. **[基本]** タブで、**[スコープ]** の下にある省略記号ボタン (...) を選択し、ドロップダウン リストからホストされた [Azure サブスクリプション] を選んで、**[選択]** を選択します。
...
1. 管理者として **WINServer** 仮想マシンにサインインします。パスワードは **Passw0rd!** です 必要に応じて。  
...
コマンド プロンプトで、次のように入力します。 各行の後で Enter キーを押して、各行にコマンドを入力します。
...
1. 左側のナビゲーション メニューで、**[操作]** まで下にスクロールし、**[コマンドの実行]** を選択します
