...
# <a name="learning-path-6---lab-1---exercise-2---connect-windows-devices-to-microsoft-sentinel-using-data-connectors"></a>ラーニング パス 6 - ラボ 1 - 演習 2 - データ コネクタを使用して Microsoft Sentinel に Windows デバイスを接続する
...
1. 下にスクロールして、仮想マシンの *[サイズ]* を確認します。 空の場合は、 **[すべてのサイズを表示]** を選択し、 *[Azure ユーザーが最もよく使用]* で最初の VM サイズを選択し、 **[選択]** をクリックします。
...
1. Windows の [スタート] ボタンを右クリックし、**[Windows PowerShell (管理者)]** を選択します。
    ...
    >**注:** 「**Windows PowerShell**」を検索する必要がある場合があります。 検索ボックスに、「**PowerShell**」と入力します。 **Windows PowerShell アプリ**が表示されるはずです。 **[管理者として実行]** オプションを選択します。
...
1. 「*.\OnboardingScript.ps1*」と入力し、Enter キーを押します。  
    ...
    >**重要:** *"用語 .\OnboardingScript.ps1 が認識されません..."* というエラーが表示された場合は、タスク 3 の手順を WIN2 仮想マシン上で行っていることを確認してください。 他の問題として、複数回ダウンロードしたためにファイルの名前が変更された可能性があります。実行中のディレクトリで *".\OnboardingScript (1).ps1"* またはその他のファイル番号を検索してください。
