...
1. **Windows 10 および 11** のオペレーティング システムの選択は、既定値のままにします。

1. [1. デバイスをオンボードする] 領域では、既定の [接続] を [合理化] に設定し、[デプロイ方法] を [ローカル スクリプト (最大 10 台のデバイス)] 設定のままにします。**********

1. **[オンボーディング パッケージのダウンロード]** ボタンを選択します。

1. [ダウンロード] ポップアップで、マウスを使用して GatewayWindowsDefenderATPOnboardingPackage.zip ファイルを選択し、フォルダー アイコン **[フォルダーに表示]** を選択します。**** **ヒント:** 表示されない場合は、ファイルを c:\users\admin\downloads ディレクトリに配置してください。

    >**ヒント:** ブラウザーでダウンロードがブロックされた場合は、ブラウザーで許可するように対処してください。 Microsoft Edge ブラウザーで、"GatewayWindowsDefenderATPOnboardingPackage.zip は一般的にダウンロードされていません。... を信頼済みであることを確認してください" というメッセージが表示されることがあります。必要に応じて省略記号ボタン (...) を選択し、**[保持]** を選択します。** Microsoft Edge では、2 つ目のポップアップが "開く前に、GatewayWindowsDefenderATPOnboardingPackage.zip を信頼済みであることを確認してください" というメッセージと共に表示されます。**[詳細を表示]** を選んで選択内容を展開し、**[そのまま保持]** を選択します。**
