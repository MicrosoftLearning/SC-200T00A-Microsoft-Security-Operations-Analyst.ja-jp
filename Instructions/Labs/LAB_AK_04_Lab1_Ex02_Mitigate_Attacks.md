...
1. **Microsoft Defender XDR** ポータルにまだアクセスしていない場合は、**Microsoft Edge** ブラウザーを起動し、**Microsoft Defender XDR** (`https://security.microsoft.com`) に移動して、お使いのテナントの管理者としてログインします。
...
1. **Microsoft Defender XDR** ポータルで、左側のメニュー バーから **[調査と対応]** を展開し、**[インシデントとアラート]** を展開して **[アラート]** を選択します。
...
1. **Microsoft Defender XDR** ポータルで、左側のメニュー バーから **[インシデントとアラート]** を選択し、**[インシデント]** を選択します
...
1. **[IP アドレス]** ページの [概要]、[インシデントとアラート]、[組織で確認] の各タブの内容を確認します。** 一部のタブには、IP アドレスに関する情報が含まれていない場合があります。
