...
# <a name="learning-path-7---lab-1---exercise-3---create-a-scheduled-query"></a>ラーニング パス 7 - ラボ 1 - 演習 3 - スケジュールされたクエリを作成する
...
1. *[Alert automation (classic)] (アラートのオートメーション (クラシック))* 領域の [自動応答] タブで、前の演習で作成したプレイブック *PostMessageTeams-OnAlert* を選択します。

1. [Automation rules (Preview)] (オートメーション ルール (プレビュー)) で、 **[新規追加]** を選択します。
...
1. *[ディレクトリ ロール]* ブレードで、 **[ユーザー管理者]** のチェック ボックスをオンにします。

1. **[追加]** を選択します。

1. コマンド バーの **[更新]** ボタンを選択して、新しいロールの割り当てを確認します。 
...
1. 新しく作成したインシデントが表示されます。 
    ...
    >**注:** アラートをトリガーするイベントは、処理に 5 分以上かかることがあります。 次の演習を続けて、後でこのポイントに戻ることができます。
