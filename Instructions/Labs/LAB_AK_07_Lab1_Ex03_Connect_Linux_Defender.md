...
# ラーニング パス 7 - ラボ 1 - 演習 3 - データ コネクタを使用して Microsoft Sentinel に Linux ホストを接続する
...
1. **[構成]** セクションで、**[+ データ収集ルールの作成]** ボタンを選択します。
    ...
    >**重要:** ルール名には一意の名前を入力する必要があります。自分の *Student* ユーザー名番号 (例: **AZLINxxxxxxxxDCR**) を使用することを検討してください。

1. **[データ収集ルールの作成]** ページの **[基本]** タブで、[ルール名] に「**AZLINxxxxxxxxDCR**」と入力し、**[次へ: リソース]** を選択します。

1. *[リソース]* タブで、*[スコープ]* の下の **[MOC サブスクリプション]** を展開します。

    >**ヒント:***[スコープ]* 列の前にある ">" を選択することで、*[スコープ]* 階層全体を展開できます。

1. **[SentinelStatic]** を展開してから、**[LIN1]** を選択します。

    >**注:** *LIN1* 仮想マシンは、ubuntuxxx などの別の名前で表示される場合があります。
...
1. データ収集ルールによって Azure Monitor エージェント (AMA) がインストールされ、CEF コレクターをインストールするための "CEF コレクター" インストール コマンドは LIN1 マシンに事前にデプロイされています。**
...
1. Microsoft Edge ブラウザーで、Microsoft Defender ポータル (`https://security.microsoft.com`) に移動します。
...
1. Microsoft Defender のナビゲーション メニューで、下にスクロールして **[Microsoft Sentinel]** を展開します。

1. Microsoft Sentinel のナビゲーション メニューで、**[コンテンツ管理]** セクションまで下にスクロールし、**[コンテンツ ハブ]** を選択します。
...
1. **[AMA 経由の Syslog]** データ コネクタを選択し、コネクタ情報ブレードで **[コネクタ ページを開く]** を選択します。

1. [構成] セクションで、**[+ データ収集ルールの作成]** を選択します。****

    >**重要:** ルール名には一意の名前を入力する必要があります。自分の *Student* ユーザー名番号 (例: **AZLINxxxxxxxxDCR2**) を使用することを検討してください。

1. **[データ収集ルールの作成]** ページの **[基本]** タブで、[ルール名] に「**AZLINxxxxxxxxDCR2**」と入力してから、**[次へ: リソース]** を選択します。

1. *[リソース]* タブで、*[スコープ]* の下の **[MOC サブスクリプション]** を展開します。

    >**ヒント:***[スコープ]* 列の前にある ">" を選択することで、*[スコープ]* 階層全体を展開できます。

1. **[SentinelStatic]** を展開してから、**[LIN2]** を選択します。
...
1. データ収集ルールによって Azure Monitor エージェント (AMA) がインストールされ、AMA Forwarder をインストールするための *AMA Forwarder* インストール コマンドは LIN2 マシン上に事前にデプロイされています。
...
1. 「**exit**」と入力して LIN2 へのリモート シェル接続を閉じます。
