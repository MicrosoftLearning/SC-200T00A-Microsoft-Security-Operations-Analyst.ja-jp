...
1. 監視拡張機能を確認します。 **[Log Analytics エージェント/Azure Monitor エージェント]** が **[オフ]** になっていることを確認します。 ページの右上にある [X] を選択して、[Settings & monitoring] (設定と監視) ページを閉じます。
...
1. ([Defender プランを選択する] の右側にある) **[すべてを有効にする]** を選択し、 **[保存]** を選択します。 "ワークスペース uniquenameDefender の Microsoft Defender プランが正常に保存されました" という** 通知が表示されるまで待ちます。
