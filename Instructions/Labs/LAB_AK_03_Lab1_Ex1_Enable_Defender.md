...
1. [クラウドのMicrosoft Defender] の左側のメニューの [管理] で、[環境設定] を選択**します**。

1. **"Azure Pass - スポンサー"** サブスクリプション (または、お使いの言語の同等の名前) を選択します。 

1. Defender for Cloud プランで保護されている Azure の要件を確認します。

1. 設定領域から **[自動プロビジョニング]** を選択します。

1. 自動プロビジョニング-拡張機能を確認します。 **Log Analytics agent for Azure VMs** が **オフ**.になっていることを確認します。
...
1. **[すべての Microsoft Defender for Cloud プランの有効化]** を選択し、 **[保存]** を選択します。 *"ワークスペース uniquenameDefender のMicrosoft Defender プランが正常に保存されました"* まで待ちます。 通知が表示されるまで待ちます。
...
1. 「cd C:\Users\Administrator\Downloads」と入力します。
    ...
    >**大事な：** このディレクトリがない場合は、間違ったコンピューターに存在している可能性が高くなります。 タスク 4 の先頭に戻るし、WINServer に変更してやり直します。
...
1. セットアップ プロセスにより、Azure Arc エージェントを認証するための新しい [Edge ブラウザー] タブが開きます。 管理者アカウントを選択し、"認証の完了" というメッセージが表示されるまで待ってから、Windows PowerShell ウィンドウに戻ります。

1. スクリプトをダウンロードした Azure portal ページに戻り、 **[閉じる]** を選択します。 **[Azure Arc を使用してサーバーを追加]** を閉じて、Azure Arc の **[サーバー]** ページに戻ります。

1. WINServer サーバー名が表示され、[状態] が *[接続*済み] になるまで **[最新の情報に更新**] を選択します。
...
1. **Log Analytics エージェントの指示を選択する**
