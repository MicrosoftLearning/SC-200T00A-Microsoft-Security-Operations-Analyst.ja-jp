---
ラボ: タイトル: 演習 1 - Microsoft Defender XDR で Microsoft Sentinel を使用して脅威の追求を実行する モジュール: ラーニング パス 9 - Microsoft Sentinel で脅威の追求を実行する 説明: ラーニング パス 9 のラボ演習で作成されたログ データは、次の前提条件タスクを再実行せずにこのラボで使用することはできません。
所要時間: 60 分 レベル: 300 islab: true primarytopics: 
    - Microsoft Sentinel
    - Azure Arc
    - Kusto 照会言語 (KQL)
    - データ レイク
...
1. 提供された資格情報を使用して **WINServer** 仮想マシンにサインインします。
...
1. *[新しいクエリ]* タブに次の KQL クエリを入力します。

   >**重要:** エラーを回避するために、最初に KQL クエリをメモ帳に貼り付け、そこから *[新しいクエリ]* タブにコピーしてください。
...
1. **[結果]** ペインのコマンド バーで、省略記号 **(...)** ボタンを選択し、ドロップダウン メニューから **[インシデントにリンク]** を選択します。
...
1. [新しい KQL ジョブの作成] ウィザードが開きます。**
    ...
    >**注:** "適用される従量課金" メッセージを確認します。**

1. [ジョブ名] フィールドに、ジョブの名前を入力します。**

1. [分析レベルの宛先テーブル] セクションで、[宛先のワークスペース] ドロップダウン メニューから **SentinelWorkspace-01** ワークスペースを選択します。****

    >**注:** *_KQL_CL* は、カスタム ログに既定で追加されるテーブルです。

1. [新しいテーブルの作成] ラジオ ボタンをオンにしたまま、新しいテーブル名として「**C2ATTACKHUNT**」と入力します。**
...
1. 結果に基づいて、仮説を検証するのに十分な強力な証拠があるかどうかを判断します。 証拠がない場合は、[ハント] を閉じて、仮設を無効としてマークします。
