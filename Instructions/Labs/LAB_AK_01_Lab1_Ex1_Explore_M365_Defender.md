...
ラボを起動すると、無料のトライアル テナントを利用して Microsoft Virtual Lab 環境にアクセスできます。 このテナントには自動的に一意のユーザー名とパスワードが割り当てられています。 Microsoft Virtual Lab 環境で Azure と Microsoft 365 にサインインするには、このユーザー名とパスワードを取得する必要があります。 
...
1. 新しいスペースが正常に完了すると、アカウント、電子メールによる通知、アラート サービス設定、アクセス許可とロールおよび Streaming API の Microsoft Defender XDR [全般] 設定が表示されます。 **[プレビュー機能]** もオンになります。
